...
class MY_GUI():
    ...
    def set_init_window(self):
        ...
        # 控件
        self.set_dir_button = Button(self.init_window_name, text="第一步：设置模型目录", bg="LightGrey", command=self.set_dir)
        self.set_dir_button.place(x=100, y=100, width=160, height=60)

        self.open_config_button = Button(self.init_window_name, text="第二步：修改配置参数", bg="LightGrey", command=self.open_config, state='disabled')
        self.open_config_button.place(x=100, y=200, width=160, height=60)

        self.run_odest_button = Button(self.init_window_name, text="第三步：运行OD识别算法", bg="LightGrey", command=self.run_odest, state='disabled')
        self.run_odest_button.place(x=100, y=300, width=160, height=60)

        self.run_assign_button = Button(self.init_window_name, text="第四步：运行客流分配算法", bg="LightGrey", command=self.run_assign, state='disabled')
        self.run_assign_button.place(x=100, y=400, width=160, height=60)

        self.open_help_button = Button(self.init_window_name, text="打开帮助文档", bg="LightGrey", command=self.open_help, state='disabled')
        self.open_help_button.place(x=300, y=100, width=160, height=60)

        self.recover_config_button = Button(self.init_window_name, text="恢复配置参数", bg="LightGrey", command=self.recover_config, state='disabled')
        self.recover_config_button.place(x=300, y=200, width=160, height=60)

        self.open_log_button = Button(self.init_window_name, text="查看日志文件", bg="LightGrey", command=self.open_log, state='disabled')
        self.open_log_button.place(x=300, y=300, width=160, height=60)

        self.open_output_button = Button(self.init_window_name, text="打开输出文件夹", bg="LightGrey", command=self.open_output, state='disabled')
        self.open_output_button.place(x=300, y=400, width=160, height=60)

        self.log_data_text = Text(self.init_window_name)
        self.log_data_text.place(x=540, y=100, width=360, height=360)
